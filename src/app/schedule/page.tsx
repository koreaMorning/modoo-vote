...
function ProgramCell({ show }: { show: ShowCell }) {
  return (
    <td>
      <div className={s.progTitle}>{show.title}</div>
      <div className={s.progGenre}>{show.genre}</div>
      <div className={s.progDesc}>{show.desc}</div>
      <div className={s.progBottom}>
...
function UpcomingCell({ show }: { show: UpcomingCell }) {
  return (
    <td>
      <div className={s.upcomingTitle}>{show.title}</div>
      <div className={s.upcomingDate}>{show.date}</div>
      <div className={s.upcomingDesc}>{show.desc}</div>
    </td>
  );
}
        ...
        <div className={s.paper}>
          ...
          <div className={s.scheduleWrap}>
            <table className={s.scheduleTable}>
              <thead>
                <tr>
                  <th className={s.thRank}>순위</th>
                  <th className={s.thPlatform}>넷플릭스</th>
                  <th className={s.thPlatform}>티빙</th>
                  <th className={s.thPlatform}>웨이브</th>
                  <th className={s.thPlatform}>디즈니+</th>
                  <th className={s.thPlatform}>쿠팡플레이</th>
                </tr>
              </thead>
              <tbody>
                {ranks.map((row) => (
                  <tr key={row.rank}>
                    <td className={s.rankCell}>
                      <div className={`${s.rankCircle} ${row.rank <= 3 ? s.rankCircleTop3 : ""}`}>
                        {row.rank}
                      </div>
                      <span className={`${s.rankChange} ${row.changeType === "up" ? s.up : row.changeType === "down" ? s.down : s.newRank}`}>
                        {row.change}
                      </span>
                    </td>
                    <ProgramCell show={row.netflix} />
                    <ProgramCell show={row.tving} />
                    <ProgramCell show={row.wavve} />
                    <ProgramCell show={row.disney} />
                    <ProgramCell show={row.coupang} />
                  </tr>
                ))}

                {/* 공개예정 구분 */}
                <tr className={s.sectionRow}>
                  <td colSpan={6}>— 공 개 예 정 작 —</td>
                </tr>

                {upcoming.map((row) => (
                  <tr key={row.month}>
                    <td className={s.upcomingMonth} style={{ whiteSpace: "pre-line" }}>{row.month}</td>
                    <UpcomingCell show={row.netflix} />
                    <UpcomingCell show={row.tving} />
                    <UpcomingCell show={row.wavve} />
                    <UpcomingCell show={row.disney} />
                    <UpcomingCell show={row.coupang} />
                  </tr>
                ))}
              </tbody>
            </table>
